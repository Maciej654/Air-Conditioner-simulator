...
def get_data(entry, backend):
    ...
    backend.dataEntered+=1 #czy wcisnieto przycisk danych

def update_time(backend,start_time):

    if (backend.dataEntered==0):
...
def show_temperature(Backend):
    if Backend.dataEntered == 1:
        Backend.carTemp = Backend.airTemp
        Backend.dataEntered+=1
    if Backend.dataEntered >= 1:
...
default_values = [1500,36, 400000,31,23,0.3]
